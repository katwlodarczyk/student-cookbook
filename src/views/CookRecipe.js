...
const CookRecipe = () => {
    ...
    const getRecipeData = async () => {
        ...
        if (docSnap.exists()) {
            ...
            getProgress((parseInt(params.stepId, 10)),docSnap.data().instructions.length)
            setLoading(false)
        } else {
            setLoading(false)
    ...
    function getCurrentStep(stepIndex) {
        setCurrentStep(stepIndex+1)
    }

    function getNextStep(stepIndex) {
        setNextStep(stepIndex+2)
    }
  ...
  return (
    <div className="relative text-gray-700 w-full min-h-screen h-max">
        ...
        { loading ? 
        (
            <div className="fixed bottom-24 w-full py-6 bg-brand-orange text-white tracking-wide font-semibold text-xl flex flex-row justify-center items-center space-x-6">
                <span>Loading</span>
            </div>
        ) : (
            <Link to={currentStep < steps ? `/recipe/${recipeId}/step/${nextStep}` : `/`} className="fixed bottom-24 w-full py-6 bg-brand-orange text-white tracking-wide font-semibold text-xl flex flex-row justify-center items-center space-x-6">
