...
import bowlIllustration from "../assets/illustrations/bowl-illustration.png";
import {
    doc,
...
const CookRecipe = () => {
    ...
    function getStepIndex(recipe, stepId) {
        const currentStepIndex = (element) => element === (stepId -1)
        setStepIndex(recipe.instructions.findIndex(currentStepIndex))
    }
    ...
    useEffect(() => {
        getRecipeData();
        getSteps(recipeId)
        getStepIndex(recipeId, parseInt(params.stepId, 10))
        getStep(recipe, stepIndex)
        getProgress(currentStep,steps)
        setLoading(false)
        // eslint-disable-next-line react-hooks/exhaustive-deps
      }, []);

    // let steps = getSteps(recipeId);
    // let stepIndex = getStepIndex(recipeId, parseInt(params.stepId, 10));
    // let step = recipe.instructions[stepIndex]
    const currentStep = stepIndex ? stepIndex - 1 : 1;
    // const progress = (currentStep / steps.length) * 100 + '%';
    const nextStep = currentStep ? currentStep +1 : 2;
    const bowl = bowlIllustration
  ...
  return (
    <div className="relative text-gray-700 w-full min-h-screen h-max">
        ...
        <div className="mb-44 p-4 flex flex-col space-y-5">
            <h1 className="font-semibold text-2xl mb-4">{recipe.title}</h1>
            <div className="text-sm">
                { loading ? 'Loading' : ('Step'+ <span>{stepIndex} / {steps}</span>)}
            </div>
            ...
            <img className="absolute z-0 bottom-56 w-full flex-none inset-x-0 opacity-30" src={bowl} alt="bowl" />
        </div>
        { loading ? 'Loading' : (
            <Link to={step.id < steps.length ? `/recipe/${recipe.id}/step/${nextStep}` : `/`} className="fixed bottom-24 w-full py-6 bg-brand-orange text-white tracking-wide font-semibold text-xl flex flex-row justify-center items-center space-x-6">
            <span>{step.id < steps.length ? 'CONTINUE' : 'FINISH'}</span>
