...
function RecipeCardFluid(props) {
    ...
    const addToWeeklyPlanner = async (date, recipeName) => {
        ...
        try {
            ...
            NotificationManager.success('Recipe has been moved', 'Success!', 2000)
        } catch (e) {
    ...
    return (
        <div className="relative w-full h-34 text-white text-xxxs shadow-lg rounded-lg flex" style={{ background: `url(${recipe.photo_url})`, backgroundSize: 'cover', backgroundPosition: 'center' }} no-repeat="true"> 
            <div className="absolute w-full h-full bg-recipe-overlay rounded-lg"></div>
            <div className={"w-full h-full p-2 flex flex-row items-end justify-between"}>
                <div className="text-base pb-1 z-10">
                    { recipe.name }
                </div>
                <div className="relative inline-block text-left z-20">
                    <svg onClick={displayMenu} className="-mb-1" width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    {showMenu && (
                        <div className="z-50 origin-top-right absolute right-0 w-40 rounded-md shadow-lg bg-white focus:outline-none">
                            <div className="z-40 py-1">
                                ...
                                <div onClick={openCalendarModal}  className="text-gray-700 block px-4 py-2 text-xs" id="1">Move day</div>
